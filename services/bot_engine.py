...
from services.qwen_ai import _delete_chat
...
class BotEngine:
    ...
    async def _close_trade_room(self, signal: dict):
        """Delete the persistent Qwen chat room when a trade closes."""
        chat_id = signal.get("trade_chat_id")
        if not chat_id:
            return
        # Pakai token yang sama dengan yang buat room — room di Qwen tied ke token pemiliknya
        token = signal.get("trade_token")
        if not token:
            logger.debug(f"[BotEngine] trade room {chat_id} — no token stored, skip delete")
            return
        try:
            import httpx
            async with httpx.AsyncClient(timeout=15) as client:
                await _delete_chat(token, chat_id, client)
            print(f"[BotEngine] trade room {chat_id} deleted for {signal.get('symbol', '?')}")
        except Exception as e:
            logger.debug(f"[BotEngine] trade room delete failed: {e}")
        signal.pop("trade_chat_id", None)
        signal.pop("trade_token", None)
            ...
            if inv_f and not entry_hit:
                ...
                if inv_hit:
                    ...
                    print(f" ❌ {sig_id} INVALIDATED @ {price}"
                          f" [active {self._active_count()}/{MAX_ACTIVE_SIGNALS}]")
                    await self._close_trade_room(signal)
                    price_feed.unwatch(symbol)
                    break
            ...
            if (
                ...
                try:
                    ...
                    ai_result = await position_ai.decide_with_retry(
                        ...
                        trade_chat_id=signal.get("trade_chat_id"),
                        trade_token=signal.get("trade_token"),
                    )

                    # Persist the trade room chat_id + token back into signal
                    if ai_result.get("chat_id"):
                        signal["trade_chat_id"] = ai_result["chat_id"]
                        signal["trade_token"] = ai_result.get("chat_token")

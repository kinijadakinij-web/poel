...
from services.qwen_ai import qwen_ai, _delete_chat
...
async def _cleanup_qwen_chat(signal: dict):
    """Hapus chat room Qwen yang dipakai untuk analisa + monitoring posisi ini."""
    chat_id = signal.get("qwen_chat_id")
    token = signal.get("qwen_token")
    if chat_id and token:
        import httpx
        try:
            async with httpx.AsyncClient(timeout=15) as client:
                await _delete_chat(token, chat_id, client)
            print(f"[bot_engine] 🗑️ Qwen chat deleted: {chat_id}")
        except Exception as e:
            print(f"[bot_engine] ⚠️ Failed to delete Qwen chat {chat_id}: {e}")
...
SCAN_BATCH_SIZE = int(os.getenv("SCAN_BATCH_SIZE", "3"))  # simbol paralel per iterasi
...
class BotEngine:
    ...
    async def _loop(self):
        ...
        while self.running:
            try:
                ...
                # Exclude symbols yang sudah punya OPEN signal agar tidak double-signal
                open_syms = self._get_open_symbols()
                pool = await self._build_pool(exclude=self._pass_scanned | open_syms)
                ...
                self.state["status"] = "RUNNING"
                print(
                    f"\n[Loop] Active={active}/{MAX_ACTIVE_SIGNALS} "
                    f"slots_needed={slots_needed} "
                    f"pool={len(pool)} batch={SCAN_BATCH_SIZE}"
                )
                if open_syms:
                    print(f"[Loop] Skipping {len(open_syms)} already-open symbol(s): "
                          f"{', '.join(sorted(open_syms))}")

                # ── Iterasi pool dalam batch SCAN_BATCH_SIZE ──────────────────
                i = 0
                while i < len(pool) and self.running:
                    if self._active_count() >= MAX_ACTIVE_SIGNALS:
                        break

                    batch_syms = pool[i:i + SCAN_BATCH_SIZE]
                    i += SCAN_BATCH_SIZE

                    print(f"\n[Batch] Scanning: {', '.join(batch_syms)}")
                    self._emit("status", f"Scanning: {', '.join(batch_syms)}")

                    # ── Fetch market data untuk semua simbol di batch secara paralel ──
                    fetch_tasks = [self._fetch_market_data(sym) for sym in batch_syms]
                    fetch_results = await asyncio.gather(*fetch_tasks, return_exceptions=True)

                    # Tandai sudah di-scan (terlepas berhasil atau tidak)
                    for sym in batch_syms:
                        self._pass_scanned.add(sym)

                    # ── Bangun daftar item yang valid untuk analisis AI ──────────
                    ai_batch = []
                    _lev = virtual_exchange.get_info(self.user_id)["leverage"]
                    for sym, result in zip(batch_syms, fetch_results):
                        if isinstance(result, Exception) or result is None:
                            logger.debug(f"[Batch] {sym} market data fetch failed: {result}")
                            continue
                        candles_by_tf, current_price = result
                        ...
                        ai_batch.append((sym, candles_by_tf, current_price, backend_ctx, _lev))
    ...
    def _process_signal(self, symbol: str, current_price: float, signal: dict) -> bool:
        ...
        signal_record = {
            ...
            # Disimpan dari qwen_ai.analyze() agar position_ai bisa pakai konteks asli
            "original_prompt": signal.get("original_prompt"),
            "original_ai_response": signal.get("original_ai_response"),
            # Chat room dari analisa awal — position_ai melanjutkan di room yang sama
            # agar AI punya memori conversational lengkap. Dihapus saat posisi ditutup.
            "qwen_chat_id": signal.get("qwen_chat_id"),
            "qwen_token": signal.get("qwen_token"),
        ...
        while time.time() - start < max_duration:
            ...
            if inv_f and not entry_hit:
                ...
                if inv_hit:
                    ...
                    print(f" ❌ {sig_id} INVALIDATED @ {price}"
                          f" [active {self._active_count()}/{MAX_ACTIVE_SIGNALS}]")
                    await _cleanup_qwen_chat(signal)
                    price_feed.unwatch(symbol)
                    break
            ...
            if not entry_hit:
                ...
                if touched:
                    ...
                    signal["entry_hit_at"] = int(time.time() * 1000)  # catat waktu entry hit
            ...
            if (
                ...
                try:
                    ...
                    ai_result = await position_ai.decide_with_retry(
                        ...
                        opened_at=signal.get("entry_hit_at") or signal.get("timestamp"),
                        original_prompt=signal.get("original_prompt"),
                        original_ai_response=signal.get("original_ai_response"),
                    ...
                    # Kalau position_ai melakukan token fallback → update room+token di signal
                    # supaya monitoring berikutnya langsung pakai room yang baru
                    if ai_result.get("_new_chat_id") is not None:
                        old_room = signal.get("qwen_chat_id")
                        signal["qwen_chat_id"] = ai_result["_new_chat_id"]
                        signal["qwen_token"] = ai_result.get("_new_token") or signal.get("qwen_token")
                        print(
                            f" 🔄 {sig_id} Qwen room updated after token fallback: "
                            f"{old_room} → {signal['qwen_chat_id']}"
                        )
    ...
    def _get_open_symbols(self) -> set:
        """
        Return set simbol yang sudah punya signal OPEN (entry_hit atau masih pending).
        Scanner akan skip simbol ini supaya tidak double-signal pada coin yang sama.
        """
        return {s["symbol"] for s in self.state["signals"] if s.get("status") == "OPEN"}

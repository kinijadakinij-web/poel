...
def _migrate_v3(conn):
    """Add entry_hit_at, original_prompt, original_ai_response columns."""
    columns_to_add = [
    ...
    for col, ctype in columns_to_add:
        if col not in existing_cols:
            conn.execute(f"ALTER TABLE signals ADD COLUMN {col} {ctype}")
            print(f"[DB] Migrated v3: added column {col} {ctype}")

def _migrate_v4(conn):
    """Add trade_chat_id and trade_token columns for persistent AI trade rooms."""
    columns_to_add = [
        ("trade_chat_id", "TEXT"),
        ("trade_token",   "TEXT"),
    ]

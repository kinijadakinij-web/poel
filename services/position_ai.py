...
class PositionAIClient:
    ...
    async def decide(
        ...
        # ── Context block — dua mode ──────────────────────────────────────────
        # MODE 1 (same room): AI sudah punya history analisa, cukup kasih tau interval.
        # MODE 2 (fallback room baru): token original limit → room ganti, inject analisa ulang.
        monitor_interval_min = MONITOR_INTERVAL // 60
        same_room_block = (
            f"\n━━━ MONITORING CONTEXT ━━━\n"
            f" You are continuing in the SAME chat room where you originally analyzed {symbol}.\n"
            f" Your full analysis and reasoning from that session are already in your conversation history above.\n"
            f" You will receive a position update every {monitor_interval_min} minute(s).\n"
            f" Each update contains fresh candle data and the latest position status.\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        )
        ...
        if original_prompt or original_ai_response:
            parts = [
                ...
                f" NOTE: This is a NEW chat room. The token that held the original room was",
                f" rate-limited and rotated. Context from the original analysis is re-injected below.",
                f" You will receive a position update every {monitor_interval_min} minute(s).",
            ]
            if original_prompt:
                preview = original_prompt[:2000] + ("\n... [truncated]" if len(original_prompt) > 2000 else "")
                parts.append(f"\n[ORIGINAL PROMPT TO ANALYSIS AI]\n{preview}")
            if original_ai_response:
                preview = original_ai_response[:2000] + ("\n... [truncated]" if len(original_ai_response) > 2000 else "")
                parts.append(f"\n[ORIGINAL ANALYSIS AI RESPONSE]\n{preview}")
            parts.append("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n")
            fallback_conv_block = "\n".join(parts)
        else:
            fallback_conv_block = (
                f"\n━━━ MONITORING CONTEXT (new room — token rotated) ━━━\n"
                f" Original analysis context unavailable (not stored).\n"
                f" You will receive a position update every {monitor_interval_min} minute(s).\n"
                f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            )
        ...
        # Prompt untuk mode same-room (tidak perlu paste analisa)
        prompt_same_room = POSITION_SYSTEM_PROMPT + "\n\n---\n\n" + \
            user_text_template.format(context_block=same_room_block)

        # Prompt untuk mode fallback (token lama limit → room baru, inject analisa)
        prompt_new_room = POSITION_SYSTEM_PROMPT + "\n\n---\n\n" + \
            user_text_template.format(context_block=fallback_conv_block)
        ...
        # ─── Chat handling ────────────────────────────────────────────────────
        # Prioritas:
        #   1. Pakai room yang sama dengan analisa (qwen_chat_id + qwen_token)
        #   2. Kalau token original limit/fail → buat room baru dengan self.token,
        #      inject analisa awal ke prompt supaya konteks tidak hilang,
        #      dan return new_chat_id + new_token agar signal diupdate.
        use_existing_room = bool(qwen_chat_id)
        active_token = qwen_token if (use_existing_room and qwen_token) else self.token

        full_text = ""
        result_chat_id = qwen_chat_id   # akan diupdate kalau room ganti
        result_token = qwen_token       # akan diupdate kalau token ganti
        ...
        async with lock:
            print(
                f"[PositionAI] lock acquired → hold/close query for {symbol} "
                f"({'reusing analysis room ' + str(qwen_chat_id) if use_existing_room else 'new room'})"
            )
            for attempt in range(2):
                try:
                    if use_existing_room and not token_fallback_used:
                        # Mode 1: lanjut di room analisa, pakai token original
                        chat_id = qwen_chat_id
                        cur_token = active_token
                        cur_prompt = prompt_same_room
                    else:
                        # Mode 2: fallback — buat room baru dengan self.token (rotated)
                        # Inject analisa awal ke prompt supaya AI punya konteks
                        ...
                        cur_prompt = prompt_new_room
                        token_fallback_used = True
                        print(
                            f"[PositionAI] ⚠️ Token rotation fallback for {symbol} — "
                            f"new room created, original analysis re-injected into prompt"
                        )

                    raw_reply = await _send_stream(
                        cur_token, chat_id, cur_prompt, self.client, uploaded_files or None
                    )

                    if token_fallback_used and not use_existing_room:
                        # Room baru yang dibuat saat fallback: jangan simpan (hapus sekarang)
                        await _delete_chat(self.token, chat_id, self.client)
                        result_chat_id = None
                        result_token = None
                    elif token_fallback_used:
                        # Room baru dibuat karena token original limit: simpan untuk monitoring berikutnya
                        result_chat_id = chat_id
                        result_token = self.token
                    ...
                    # Reply kosong → kalau attempt 0 dan pakai room lama, coba fallback
                    if attempt == 0:
                        if use_existing_room and not token_fallback_used:
                            print(f"[PositionAI] Empty reply from original room — trying token fallback")
                            token_fallback_used = True
                        continue
                    return None

                except httpx.TimeoutException:
                    if attempt == 0 and use_existing_room and not token_fallback_used:
                        print(f"[PositionAI] Timeout on original room for {symbol} — trying token fallback")
                        token_fallback_used = True
                        continue
                    ...
                    if attempt == 0 and use_existing_room and not token_fallback_used:
                        print(f"[PositionAI] Error on original room for {symbol} ({e}) — trying token fallback")
                        token_fallback_used = True
                        continue
        ...
        return {
            ...
            "reason": reason,
            "new_sl": new_sl,
            # Kalau room/token berubah saat fallback, bot_engine harus update signal
            # agar monitoring berikutnya pakai room + token yang baru.
            # None berarti tidak ada perubahan.
            "_new_chat_id": result_chat_id if result_chat_id != qwen_chat_id else None,
            "_new_token": result_token if result_token != qwen_token else None,
        }

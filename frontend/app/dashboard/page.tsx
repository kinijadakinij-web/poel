...
export default function DashboardPage() {
  ...
  const handleReset = async () => {
    ...
    try {
      // Stop bot dulu supaya tidak langsung generate signal baru setelah reset
      if (botRunning) {
        try { await stopBot(); setBotRunning(false); } catch {}
      }
      const res = await resetBot();
      if (res.ok) {
        // Langsung clear UI — jangan tunggu WS atau polling
        dispatch({ type: "SET_STATE", payload: {
          signals: [], trade_count: 0, win_count: 0, loss_count: 0,
          no_trade_count: 0, total_pnl_pct: 0, total_pnl_usdt: 0,
          symbols_scanned: 0, active_signal_count: 0, status: "IDLE",
        } as any });

...
export function TradingProvider({ children }: { children: React.ReactNode }) {
  ...
  // ── fetchData diangkat ke useCallback supaya bisa dipakai WS dan polling ──
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      ws.onopen = () => {
        ...
        // Setelah reconnect (Railway restart / network blip),
        // langsung sync state dari backend supaya data stale tidak tertinggal di UI
        if (!isFirstConnect) {
          fetchData();
        }
    ...
    const handleEvent = (msg: WSEvent) => {
      switch (msg.event) {
        ...
        case "balance_update":
          ...
          break;
        case "reset_all":
          // 1. Langsung clear state UI sekarang juga
          dispatch({ type: "SET_STATE", payload: initialState });
          // 2. Re-fetch dari backend setelah 1.5 detik sebagai verifikasi —
          //    jaga-jaga kalau bot masih running dan sudah generate signal baru
